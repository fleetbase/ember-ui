...
/**
 * Modal::Layouts::OptionPrompt example adapter.
 *
 * The documentation is explicit that consumers do not render modal layouts directly — they call
 * `modalsManager.userSelectOption()`, which shows the layout for them. The playground demonstrates the
 * documented path rather than a shortcut, so this adapter opens the dialog through the service.
 */
export default class PlaygroundExampleModalLayoutsOptionPromptComponent extends Component {
    @service modalsManager;

    @action open() {
        this.args.onEvent?.('userSelectOption');

        return this.modalsManager.userSelectOption({
