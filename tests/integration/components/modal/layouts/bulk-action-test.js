...
// bulk-action is a pass-through layout: it renders Modal::Default with the options
...
module('Integration | Component | modal/layouts/bulk-action', function (hooks) {
    ...
    test('it renders the title from options through Modal::Default', async function (assert) {
        this.set('options', { title: 'Apply to selected' });

        await render(hbs`<Modal::Layouts::BulkAction @options={{this.options}} />`);

        assert.dom('.flb--default-modal').exists('it delegates to the default modal chrome');
        assert.dom('.flb--modal-header').containsText('Apply to selected');
    });
    ...
    test('@onConfirm is wired to the accept button', async function (assert) {
        ...
        await render(hbs`<Modal::Layouts::BulkAction @options={{this.options}} @onConfirm={{this.onConfirm}} />`);
    ...
    test('@onDecline is wired to the decline button', async function (assert) {
        ...
        await render(hbs`<Modal::Layouts::BulkAction @options={{this.options}} @onDecline={{this.onDecline}} />`);
