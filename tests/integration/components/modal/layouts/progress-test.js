...
// progress is a pass-through layout: it renders Modal::Default with the options
...
module('Integration | Component | modal/layouts/progress', function (hooks) {
    ...
    test('it renders the title from options through Modal::Default', async function (assert) {
        this.set('options', { title: 'Uploading' });

        await render(hbs`<Modal::Layouts::Progress @options={{this.options}} />`);

        assert.dom('.flb--default-modal').exists('it delegates to the default modal chrome');
        assert.dom('.flb--modal-header').containsText('Uploading');
    });
    ...
    test('@onConfirm is wired to the accept button', async function (assert) {
        ...
        await render(hbs`<Modal::Layouts::Progress @options={{this.options}} @onConfirm={{this.onConfirm}} />`);
    ...
    test('@onDecline is wired to the decline button', async function (assert) {
        ...
        await render(hbs`<Modal::Layouts::Progress @options={{this.options}} @onDecline={{this.onDecline}} />`);
