...
const isDropdownActions = function (dd) {
  return Object.keys(dd).includes(
    'uniqueId',
    'isOpen',
    'disabled',
    'actions',
...
export default Helper.extend({
  ...
  compute(positional) {
    assert(
      `You must pass a DropdownActions instance as the \`dropdown-fn\` helpers first argument, check the yield of the Dropdown component`,
      isDropdownActions(positional[0])
    );
    assert(
      `You must pass a function as the \`dropdown-fn\` helpers second argument, you passed ${positional[1]}`,
      typeof positional[1] === 'function'
    );
